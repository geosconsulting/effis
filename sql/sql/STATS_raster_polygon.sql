...
SELECT ST_Band(rast,1) FROM public.glc_1x1k WHERE rid IN (10,25,60);

SELECT PostGIS_Full_Version();

SELECT b.id,(ST_SummaryStats(a.rast, 1, true)).count
FROM public.glc_1x1k AS a, public.countries_adminsublevel1 as b
...
FROM (SELECT *
      ...
      public.glc_1x1k as raster;

SELECT * FROM fires_2001_centre;
...
SELECT rid, ST_AsText(rast::geometry) FROM glc_1x1k;
SELECT ST_AsText(ST_Buffer(ST_Union(rast::geometry), 0.000001)) FROM glc_1x1k;
...
FROM (SELECT rid, ST_SummaryStats(rast) As stats
      FROM public.glc_1x1k  --CROSS JOIN generate_series(1,3) As band
      WHERE rid=2) As foo;
...
SELECT 
    id,
    Min((gv).val) As MinLandCoverClass,
    Max((gv).val) As MaxLandCoverClass
FROM (
    SELECT 
        id,
